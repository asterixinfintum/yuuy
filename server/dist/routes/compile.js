...
var _cleanCompilations = _interopRequireDefault(require("../utils/cleanCompilations"));
var _compileContract = _interopRequireDefault(require("../utils/compileContract"));
...
function getAllContractFiles(dirPath) {
  ...
  files.forEach(function (file) {
    var filePath = _path["default"].join(dirPath, file);
    if (_fs["default"].statSync(filePath).isDirectory()) {
      allFiles = allFiles.concat(getAllContractFiles(filePath)); // Recursive call
    } else if (_path["default"].extname(file) === '.sol') {
      allFilesTwo.push(filePath);
    }
  });
  return [].concat(_toConsumableArray(allFiles), allFilesTwo);
}
function getCombinedSourceCode(files) {
  var combinedCode = '';
  files.forEach(function (file) {
    combinedCode += '\n\n'; // File: ${file}\n;
    combinedCode += _fs["default"].readFileSync(file, 'utf8');
  });
  return combinedCode;
}
var compileRoute = (0, _express["default"])();
var folderPath = _path["default"].join(__dirname, '../../serverf/contracts');
var flattenedPath = _path["default"].join(__dirname, '../../serverf/flattened');
compileRoute.post('/compile', /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(req, res) {
    var currentContent, fileName, filePath;
              ...
              var _ref2 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(err) {
                ...
                return _regeneratorRuntime().wrap(function _callee$(_context) {
                  while (1) switch (_context.prev = _context.next) {
                    ...
                    case 3:
                      (0, _compileContract["default"])();
                      files = getAllContractFiles(folderPath);
                      combinedCode = getCombinedSourceCode(files);
                      lines = combinedCode.split('\n');
                      codeFilter = lines.filter(function (line) {
                        return !line.includes("import") && !line.includes("pragma solidity 0.8.17;") && line.length && !line.includes("// SPDX-License-Identifier");
                      });
...
var _default = exports["default"] = compileRoute;